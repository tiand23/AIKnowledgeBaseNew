...
export async function uploadChunk(payload: {
  ...
  const resp = await http.post<ApiResponse<UploadChunkRespData>>("/api/v1/upload/chunk", form, {
    headers: { "Content-Type": "multipart/form-data" }
  });
  return resp.data;
}

export async function getUploadStatus(fileMd5: string) {
  const resp = await http.get<ApiResponse<UploadStatusData>>("/api/v1/upload/status", {
    params: { file_md5: fileMd5 }
  });
  return resp.data;
}

export async function mergeFile(fileMd5: string, fileName: string) {
  const resp = await http.post<ApiResponse<MergeData>>("/api/v1/upload/merge", {
    file_md5: fileMd5,
    file_name: fileName
  });
  return resp.data;
...
export async function getUserUploadedFiles() {
  const resp = await http.get<ApiResponse<UploadedFileInfo[]>>("/api/v1/documents/uploads");
  return resp.data;
}

export async function getEsPreview(fileMd5: string, size = 5) {
  const resp = await http.get<ApiResponse<EsPreviewItem[]>>("/api/v1/documents/es-preview", {
